...
**Version**: 2.8 (Bidirectional Linking)
...
- `LinkingService.bas` - Bidirectional sync between overview and KW sheets (NEW in V2.8)
...
│   ├── LinkingService.bas       # Bidirectional sync (NEW in V2.8)
...
**Module**: NEW (Version 2.8+)

**Purpose**: Bidirectional synchronization between Personalplaner (overview) and KW sheets (weekly view)

**Key Features**:
- **Automatic Sync**: Changes in overview instantly appear in corresponding KW sheet (if exists)
- **Bidirectional**: Changes in KW sheet sync back to overview
- **Code Transformation**: ShortForm ↔ LongForm mapping (e.g., "F" ↔ "Ferien")
- **Project Sync**: Project names remain identical in both sheets
- **Performance**: O(1) lookup via employee number and KW calculation
- **Loop Prevention**: Module-level flag prevents infinite sync loops

**Public Functions**:

#### `SyncCellToOverview(kwSheet As Worksheet, changedCell As Range)`
Synchronizes a cell change from KW sheet back to Personalplaner overview
- Triggered by `Tabelle7.Worksheet_Change` event
- Finds employee by number (column 1 in KW sheet)
- Calculates date from KW number and weekday offset
- Transforms LongForm → ShortForm (e.g., "Ferien" → "F")
- Writes to corresponding cell in Personalplaner
- **Performance**: < 0.5 seconds

#### `SyncCellToWeeklySheet(overviewSheet As Worksheet, changedCell As Range)`
Synchronizes a cell change from Personalplaner to corresponding KW sheet
- Triggered by `Tabelle3.Worksheet_Change` event
- Calculates KW number from date (row 10)
- Checks if KW sheet exists (e.g., "KW49 2025")
- Finds employee by number (column 6 in overview ListObject)
- Transforms ShortForm → LongForm (e.g., "F" → "Ferien")
- Writes to corresponding cell in KW sheet
- **Performance**: < 0.5 seconds

**Private Helper Functions**:
- `MapAbsenceCodeToShortForm(longForm)`: "Ferien" → "F"
- `MapAbsenceCodeToLongForm(shortForm)`: "F" → "Ferien"
- `FindEmployeeRowByNumber(sheet, employeeNumber)`: Lookup by employee number
- `ParseKWSheetName(sheetName, kwNumber, year)`: Extracts KW and year from sheet name
- `GetDateFromKWAndWeekday(year, kwNumber, weekdayOffset)`: Calculates date from KW

**Data Transformation Rules**:
| Overview (Personalplaner) | KW Sheet (Weekly View) |
|---------------------------|------------------------|
| "F" (ShortForm) | "Ferien" (LongForm) |
| "K" (ShortForm) | "Krank" (LongForm) |
| "U" (ShortForm) | "Unfall" (LongForm) |
| "ProjektX" | "ProjektX" (unchanged) |

**Important Notes**:
- Only syncs columns E-I (Monday-Friday) in KW sheets
- Only syncs columns O+ (calendar) in Personalplaner
- Syncs only when changes occur in data tables (ListObjects)
- Uses `syncInProgress` flag to prevent infinite loops
- Works with existing Worksheet_Change events (S/T recurrence, auto-fill)
...
✅ **Bidirektionale Verlinkung** - Added LinkingService for automatic sync (V2.8)
...
| **Bidirectional sync (NEW)** | **LinkingService.bas** |
